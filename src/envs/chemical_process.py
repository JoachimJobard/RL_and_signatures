...
if __name__ == "__main__":
    ...
    state_array_sin_command = jnp.stack(state_list)
    reward_array_sin_command = jnp.array(reward_list)
    time_arraysin_command = jnp.array(time_list)
    ...
    for _ in range(200):
        action = jnp.zeros((2,))  # No control
        # action = jnp.sin(0.1 * t) * jnp.array([1.0, 1.0])  # Example control input
        t, x, reward = env_wrapped.step(env_wrapped.state, action)
        state_list.append(x)
        time_list.append(t)
        reward_list.append(reward)
    
    state_array_zero_command = jnp.stack(state_list)
    reward_array_zero_command = jnp.array(reward_list)
    time_array_zero_command = jnp.array(time_list)
    
    # Plot 1: State evolution
    plt.figure(figsize=(12, 4))
    plt.subplot(1, 2, 1)
    for i in range(4):
        plt.plot(time_array_zero_command, state_array_zero_command[:, i], label=f'State x{i+1}(t)')
    plt.xlabel("Time")
    plt.ylabel("State")
    ...
    # Plot 2: Reward
    plt.subplot(1, 2, 2)
    plt.plot(time_array_zero_command, reward_array_zero_command, label='Reward')
    plt.xlabel("Time")
    plt.ylabel("Reward")
    ...
    print(f"\nFinal state: {state_array_sin_command[-1, 0]:.4f}")
    print(f"Final error from target: {abs(state_array_sin_command[-1, 0]):.4f}")
    
    plt.figure(figsize=(12, 4))
    plt.plot(time_array_zero_command, state_array_zero_command, label='No Control', linestyle='--')
    plt.plot(time_arraysin_command, state_array_sin_command, label='Sinusoidal Control', )
    plt.xlabel("Time")
    plt.ylabel("State")
    plt.title("Chemical Reaction Dynamics: Control Comparison")
    plt.legend()
    plt.show()

    plt.figure()
    for i in range(4):
        plt.plot(time_array_zero_command, state_array_zero_command[:, i], label='$x_{%d}(t)$' % (i+1))
    plt.xlabel("Time")
    plt.ylabel("State")
    plt.legend()
    plt.savefig("../master_thesis/kth-typst-template/figures/chemical_process_dynamics_no_control.pdf")
    plt.show()
