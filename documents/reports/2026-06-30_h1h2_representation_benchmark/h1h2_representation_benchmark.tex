...
J=\int_{0}^{T}\ell\big(x(t),u(t)\big)\,\mathrm dt
\end{equation}
...
\[
\hat\theta=\Big(G+\lambda\,\tfrac{\tr G}{\dim\phi}\,\mathrm{Id}\Big)^{-1}\Phi^{\top}y,
\]
...
linearisation $A=-J,\,A_1=W$ is unchanged: $H_1$ is held fixed while the value becomes a
...
H_2^{\mathrm{pop}}:\qquad \inf_{V\in\mathcal H_{\phi_S}}J(V)\;<\;\inf_{V\in\mathcal H_{\phi_H}}J(V).
\end{equation}
...
H_2^{(n)}:\qquad
\E_{\mathcal D\sim\mathcal P^{\otimes n}}\!\big[J(\hat V_{\phi_S}^{\mathcal D})\big]\;<\;
\E_{\mathcal D\sim\mathcal P^{\otimes n}}\!\big[J(\hat V_{\phi_H}^{\mathcal D})\big],
\end{equation}
...
\E\big[J(\hat V_\phi)\big]-J^{\ast}=
\underbrace{\Big(\inf_{V\in\mathcal H_\phi}J(V)-J^{\ast}\Big)}_{\text{approximation}}
+\underbrace{\Big(\E\big[J(\hat V_\phi)\big]-\inf_{V\in\mathcal H_\phi}J(V)\Big)}_{\text{estimation (conditioning)}}.
\end{equation}
...
\toprule
control & setting & $n/\dim\phi_H$ & $\eta_H$ & $J_H$ & $J_S$\\
\midrule
...
\toprule
$\tau$ ($J^{0}\!\to\! J^{\ast}$) & rep & $\dim\phi$ & $n/\dim$ & $J$ (mean $\pm$ ci)\\
\midrule
...
$\dim\phi_S/\effrank C_S\approx12$, \emph{fixed} in the delay (Table~\ref{tab:effrank}: dimension
