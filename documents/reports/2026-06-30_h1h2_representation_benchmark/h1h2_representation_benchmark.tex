...
capacity (polynomial degree, signature depth) across many cells and five seeds, and a
\emph{controlled} gated continuous-time optimal-control oracle protocol that isolates the
representational question by solving the fit exactly. (The originally-intended nonlinear-delayed
cell, a continuous-stirred-tank reactor, is measured to be structurally near-Markovian and is
...
single-seed evidence that the signature beats the history window on long-delay cells is shown
...
long-delay cell at fixed sample size the history fit is starved. Under three controls ---
quadrupled data, Tikhonov conditioning, and off-sheet rebalancing --- the history
representation recovers and \emph{matches or beats} the signature on every cell measured
...
its data protocol, and the three metrics. The per-cell sections that follow vary only the
plant.}
...
Let $V^{\star}$ be the optimal cost-to-go (value) of \eqref{eq:plant}--\eqref{eq:cost}, in the
reward convention $V^{\star}=-\,(\text{cost-to-go})$. Doya's continuous-time law sets the
control to the half-gradient of the value along the input channel,
\begin{equation}\label{eq:doyalaw}
u=\half\,R^{-1}B^{\top}\,\partial_{x}V^{\star},\qquad B:=\partial_{u}f,
\end{equation}
...
\subsection{The reference: a gated continuous-time oracle}

\begin{definition}[Gated continuous-time oracle]\label{def:oracle}
The reference controller is the continuous-time optimal control of \eqref{eq:plant}, built per
cell and \emph{gated} before use:
\begin{itemize}
\item \emph{Linear / linearised cells.} The continuous delayed-LQR, obtained by Chebyshev
collocation of the history generator on $[-\tau,0]$: the augmented (collocated) state
$z\in\R^{n(N+1)}$ carries the discretised history, an algebraic Riccati equation on the
augmented $(M,\mathcal N)$ yields $V^{\star}(z)=-z^{\top}P_{c}z$ and
$u^{\star}=-K_{c}z$. (\texttt{src/solvers/continuous\_oracle.py}.)
\item \emph{Nonlinear cells.} The Pontryagin two-point boundary-value problem on the augmented
$(z,\lambda)$: the costate $\lambda(t)=\partial_{z}V^{\star}$ is the value gradient (maximum
principle), so $u^{\star}=-\half R^{-1}B^{\top}\lambda_{0}$ is Doya's law evaluated on the
exact value. (\texttt{src/solvers/\{mackey\_glass,delayed\_hopfield\}\_pontryagin.py}.)
\end{itemize}
The gate is a magnitude/direction test: on a state cloud the oracle gradient fed through
\eqref{eq:doyalaw} must reproduce $u^{\star}$ with cosine $1$ and ratio $\norm{u}/\norm{u^{\star}}=1$.
Every cell in this report passes the gate to four decimals (the \texttt{gate} field of each
\texttt{summary.json}); a cell whose oracle failed the gate would be excluded.
\end{definition}
...
training data with $(V^{\star},u^{\star})$, and it supplies the deployable linear-feedback
reference cost $I^{\star}$ against which the learned $I$ is read.

\subsection{The two hypotheses}

\begin{definition}[$H_1$ and $H_2$]\label{def:hyp}
With the three representations of Definition~\ref{def:reps} ranked markovian $\to$ raw-history
$\to$ signature, the hypotheses are the two adjacent pairwise comparisons of closed-loop cost:
\[
H_1:\quad I[\text{raw-history}]<I[\text{markovian}]\quad(\text{``does history help?''}),
\]
\[
H_2:\quad I[\text{signature}]<I[\text{raw-history}]\quad(\text{``does the signature structure help?''}).
\]
A hypothesis is recorded as \emph{holding} on a cell only if the contender lowers $I$ by more
than a relative margin of $3\%$; a near-tie is the negative result. The single-cell verdict is
indicative; the falsifiable statement is the seed-resolved comparison of
Section~\ref{sec:h1}.
\end{definition}
...
A control-cadence \emph{window} $W=(x_{t-(L-1)\Delta t},\dots,x_{t})\in\R^{L\times n}$ collects
the last $L$ samples of the state at the control step $\Delta t$, with $L=\lceil\tau/\Delta t\rceil+1$
so it spans the delay. The value critic is linear in a feature map $\phi$,
$V_{\theta}(W)=\inner{\theta}{\phi(W)}$, with the three maps
\begin{enumerate}
\item \emph{markovian} $\phi_{M}$: the degree-$2$ monomials of the \emph{current} state $x_{t}$
alone (no history);
\item \emph{raw-history} $\phi_{H}$: the degree-$2$ monomials of the \emph{whole} window $W$
(flattened to $\R^{Ln}$);
\item \emph{signature} $\phi_{S}$: the depth-$2$ path signature of the time-augmented window
$\widehat W=(t,W)$.
\end{enumerate}
$\phi_{M}$ is the $H_1$ null (a state-only critic); $\phi_{H}$ adds history linearly in the
window; $\phi_{S}$ adds the nonlinear path-functional structure. Their feature dimensions are
tabulated per cell in Section~\ref{sec:catalogue}; the contrast in how those dimensions grow is
...
The training set is a set of labelled windows $(W,V^{\star}(W))$ of two kinds, generated by a
single procedure uniform across cells (\texttt{run/study/h1h2\_evaluation.py}).
\begin{itemize}
\item \emph{On-sheet}: windows sampled along the \emph{optimal} (oracle) trajectory from a
cloud of initial conditions --- the data a deployed controller would actually visit.
\item \emph{Off-sheet}: an on-sheet window perturbed by an isotropic displacement of scale
$\varepsilon=0.25$ and \emph{re-labelled through the oracle} (analytically for the
linear/linearised cells, by one Pontryagin boundary-value problem each for the nonlinear
cells). Off-sheet windows carry the off-manifold value, hence constrain the value
\emph{gradient} transverse to the trajectory --- the quantity \eqref{eq:doyalaw} deploys.
\end{itemize}
The sample size satisfies $n_{\text{train}}>\dim\phi$ in every reported fit; the ratio
$n_{\text{train}}/\dim\phi$ is reported because it is the binding quantity for $H_2$.
\end{definition}

\begin{definition}[Estimators]\label{def:estimator}
Two estimators of $\theta$ are used, sharing the least-squares core.
\emph{Oracle half} (the primary evidence): plain least squares of $V_{\theta}$ to the oracle
labels $V^{\star}$, $\theta=\arg\min_{\theta}\sum_i(\inner{\theta}{\phi(W_i)}-V^{\star}(W_i))^{2}$,
optionally with a scale-aware Tikhonov term $\lambda\,\tr(G)/\dim\phi$ on the Gram
$G=\Phi^{\top}\Phi$ (the knob \texttt{H1H2\_RIDGE}; $\lambda=0$ is plain least squares).
\emph{Learning half} (the confirmation, Appendix~\ref{app:rl}): damped least-squares policy
iteration (LSPI/LSTD), the temporal-difference analogue of the same estimator, on transitions
collected on-policy with exploration. Both restore the best iterate.
\end{definition}

\subsection{The three metrics}\label{sec:metrics}

\begin{definition}[Metrics]\label{def:metrics}
...
determination of $V_{\theta}$ against $V^{\star}$ on the training windows;
\item gradient cosine $\gamma:=\cos\!\big(u_{\theta},u^{\star}\big)\in[-1,1]$ ---
...
\item closed-loop cost $I$ (Definition~\ref{def:plant}) --- \emph{control}: the operative
verdict quantity, to be read against the no-control cost $I^{0}$ and the oracle-feedback cost
$I^{\star}$.
\end{itemize}
The three are ordered by stringency: $R^{2}\to1$ is necessary but far from sufficient for
$\gamma\to1$, which is in turn necessary but not sufficient for $I\to I^{\star}$. The gap between
$R^{2}=1$ and a wrong $I$ is the value/gradient gap of Section~\ref{sec:mechanism}.
\end{definition}

\section{Testbed catalogue (controlled-protocol cells)}\label{sec:catalogue}

\emph{This is the roster of the controlled gated-oracle protocol of
Sections~\ref{sec:h1}--\ref{sec:h2}, complementary to the comprehensive learned cells of
Section~\ref{sec:mainunified} (\texttt{linear\_cell}, \texttt{delayed\_velocity},
\texttt{delayed\_oscillator}, \texttt{strongly\_delayed}, \texttt{dadebo}). The eight cells here
populate a falsifiability grid: from a Markovian negative control (where $H_1$ must fail),
through linear delay cells (where $H_1$ holds and $H_2$ is a provable negative), to nonlinear
delay cells (where $H_2$ could in principle hold). The delay-coupled Hopfield network is the
nonlinear-delayed primary testbed that replaces the demoted Dadebo cell
...
\begin{definition}[The cells]\label{def:cells}
\leavevmode
\begin{itemize}
\item \textbf{markovian} (negative control). The double integrator $\ddot x=u$ on $\R^{2}$, no
delay; $V^{\star}$ depends on $x(t)$ alone, so history is irrelevant by construction. Oracle:
the standard continuous algebraic Riccati equation.
\item \textbf{linear\_dde}. The scalar linear delay equation
$\dot x(t)=-1.5\,x(t-\tau)+u$, $\tau=1$; $V^{\star}$ is exactly quadratic in the augmented
...
nonlinear functional of the history. The delay $\tau$ is the operative knob for $H_2$ (it sets
...
high-\emph{state-dimension} cell. Oracle: delayed-LQR (linearised).
\item \textbf{mg\_limit\_cycle}, \textbf{mg\_chaotic}. The scalar Mackey--Glass oscillator
$\dot x=-\mu x+p\,x(t-\tau)/(1+x(t-\tau)^{10})+u$, regulating to $x^{\star}=1$, at $\tau=6$
(limit cycle) and $\tau=17$ (chaos). Long-\emph{delay} cells. Oracle: Pontryagin.
\end{itemize}
\end{definition}

\begin{table}[ht]\centering\small
\caption{Cell parameters and feature dimensions. $n$ is the state dimension, $\tau$ the
...
\toprule
cell & $n$ & $\tau$ & $\Delta t$ & $L$ & $\dim\phi_M$ & $\dim\phi_H$ & $\dim\phi_S$ & oracle\\
\midrule
...
10--11): each cell is trained end-to-end (the value-gradient agent, semi-gradient temporal
...
normalised sub-optimality $\rho:=(J-J^{\star})/\abs{J^{\star}}$ against the per-cell delayed-LQR
oracle cost $J^{\star}$ (raw cost $J$ for the nonlinear cells, where no closed-form $J^{\star}$
...
\begin{table}[ht]\centering\small
\caption{Comprehensive learned benchmark (\texttt{main\_unified}): per-cell, per-capacity mean
metric over five seeds. Metric S${}=$ normalised sub-optimality $\rho$ (linear cells, lower
better, $0={}$oracle); C${}=$ raw closed-loop cost (nonlinear cells). Within each cell the
ordering is invariant to the metric (shared per-cell baseline). Bold marks the best raw-history
...
\toprule
cell & m & mark & raw$_1$ & raw$_2$ & sig$_2$ & sig$_3$ & sig$_4$ & $H_1$ & $H_2$\\
\midrule
...
cells is the under-sampling illusion overturned under control in Section~\ref{sec:h2}.
$^{\ddagger}$On the ill-conditioned cells the learned raw-history is \emph{under-converged}, not
out-represented (see Finding~\ref{find:underconv}). $^{\S}$Dadebo is near-Markovian (the
markovian critic is the best representation); it is demoted to a stress cell
(Remark~\ref{rem:dadebo}).}
\end{table}

\begin{finding}[$H_1$ holds strongly on the genuinely history-dependent cells]\label{find:mu-h1}
On the cells where the delay couples the dynamics the learned markovian critic is one to three
...
The originally-intended nonlinear-delayed cell, the Dadebo continuous-stirred-tank reactor, is
...
critic ($0.027$) is the \emph{best} representation on this cell, below every history and
signature variant: it does not exercise $H_1$. It is therefore demoted to a stress/robustness
cell, and the delay-coupled Hopfield network (Section~\ref{sec:h2}) replaces it as the
nonlinear-delayed primary testbed.
\end{remark}

\begin{finding}[On ill-conditioned cells the learned $H_1$ failure is under-convergence]\label{find:underconv}
On the high-gap oscillator and the long-window linear cell the learned pipeline reports $H_1$
...
$R^{2}\to1$ for raw-history on these cells --- it \emph{spans} the value --- and recovers
...
markovian one by orders of magnitude on these cells, so semi-gradient temporal difference needs
...
the higher-dimensional map is starved. Signature depth past the optimum degrades sharply
...
\emph{The controlled cross-check is the canonical oracle-half run: five cells, five seeds each,
on${}+{}$off-sheet data, plain least squares to the gated oracle value
...
the representational question. The Markovian cell is the negative control; the four delay cells
are the test. The learning-half confirmation is in Appendix~\ref{app:rl}.}

\subsection{The negative control}

\begin{finding}[The Markovian cell falsifies a spurious $H_1$]\label{find:negcontrol}
...
labels pin the history coefficients of $\phi_H$ to zero because $V^{\star}$ carries no history
...
On on-sheet-only data the same cell gives raw-history $I=3.1939\pm0.0385$ (a $2.5\%$
...
squares). $I^{0}$ is the no-control cost, $I^{\star}$ the oracle-feedback cost. Lower $I$ is
better. Bold marks the lower of raw-history vs markovian ($H_1$) and signature vs raw-history
($H_2$).}\label{tab:h1}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccc c}
\toprule
cell ($I^{0}\!\to\! I^{\star}$) & rep & $R^{2}$ & $\gamma$ & $I$ & verdict\\
\midrule
...
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize $^{\dagger}$The signature ``win'' on the long-delay Mackey--Glass cells is the
...
dimension $2484$ is starved at this sample size --- so it falls below even the markovian
critic; this is not evidence against history.}
\end{table}

\begin{finding}[$H_1$ holds wherever the delay couples the dynamics]\label{find:h1}
On every cell where the delay genuinely enters the dynamics, the history window lowers the
...
starved here --- exactly the phenomenon Section~\ref{sec:h2} controls for.
\end{remark}
...
(\texttt{20260616\_161649\_on\_sheet\_array}) leaves the low-dimensional cells intact but makes
...
\emph{This is the overturned headline. The measured empirical findings (the apparent signature
wins; their collapse under controls) are kept strictly separate from the corrected mechanism
(Section~\ref{sec:mechanism}), so that refuting the latter would not cost the former. The
controls below are all on \texttt{hopfield\_nonlinear} (the cell whose delay $\tau$ is a free
knob) and the Mackey--Glass cells (a different plant family, to test generality).}

\subsection{The apparent win}
...
\begin{finding}[The signature ``wins'' only where the history fit is starved]\label{find:apparent}
...
$H_2$ win is a statement about sample size, not representation --- a hypothesis to be controlled,
not a result.
\end{finding}

\subsection{Three controls collapse the win}
...
quadrupled data; (b) Tikhonov conditioning at fixed (starved) data; (c) off-sheet rebalancing
...
\multicolumn{6}{l}{\emph{(a) more data --- Hopfield $\tau=2$, $\dim\phi_H=945$ (\texttt{h1h2\_tau2\_control})}}\\
 baseline (starved) & $n_{\mathrm{ic}}30,\ n_{\mathrm{off}}500$  & 4.4  & 0.166 & $544.6$ & $0.264$\\
 $\times4$ data      & $n_{\mathrm{ic}}120,\ n_{\mathrm{off}}2000$ & 17.5 & 0.984 & $\mathbf{0.240}$ & $0.229$\\
\midrule
\multicolumn{6}{l}{\emph{(b) Tikhonov conditioning --- Hopfield $\tau=2$, starved data (\texttt{h1h2\_tau2\_ridge})}}\\
...
\begin{finding}[Every control overturns the apparent win]\label{find:controls}
...
(b) At fixed starved data, Tikhonov conditioning alone restores it ($I:544.6\to0.238$ at
...
and, in excess, they \emph{drown} the off-sheet gradient constraint in the least-squares normal
equations. What conditions the fit is the \emph{balance} off/dim, not $n_{\text{train}}$:
...
\begin{finding}[$H_2$ fails on a well-sampled nonlinear cell]\label{find:duffing}
On the Duffing cell, where the delay is short ($\dim\phi_H=90$) so the history window is
...
sufficient for $H_2$: when the history fit is not starved, the degree-two polynomial of the
...
$H_2$-holds verdict on this cell (Table~\ref{tab:h1}, $\dagger$) is thus an off-sheet-budget
artefact: at off/dim${}=4$ it reverses. The chaotic cell (window $69$, $\dim\phi_H=2484$, the
...
solves). The reversal thus holds on both Mackey--Glass cells and both Hopfield delays: the
signature's long-delay advantage is under-sampling of the history window, removed by off-sheet
data balanced to its dimension, with no cell measured as an exception.
\end{finding}

\begin{observation}[Verdict on $H_2$]\label{obs:h2}
Across every cell measured, the signature's advantage over the history window appears only where
the history feature dimension is starved of data, and is removed by data, by conditioning, or by
off-sheet rebalancing. On well-sampled cells (Duffing, short-delay Hopfield) and on the
high-state-dimension cell (platoon) the history window is at least as good, and the signature is
...
Let the law of the window be supported on a family $\{W^{(z)}:z\in\R^{k}\}$ indexed by a
$k$-dimensional parameter $z$ (for a regulated linear closed loop, $z$ is the low-dimensional
initial condition and $W^{(z)}$ is affine in $z$). If each depth-$m$ signature coordinate
$z\mapsto\sig^{m}(W^{(z)})$ is a polynomial of degree $\le m$ in $z$, then the covariance of the
depth-$L$ signature features has rank bounded by the dimension of the Veronese embedding,
\[
\rank\Cov\big(\sig_{\le L}(W)\big)\ \le\ \binom{k+L}{L},
\]
...
Let $\hat\ell$ minimise the empirical value risk $\sum_i(\inner{\ell}{\Phi_i}-V^{\star}_i)^{2}$ and
...
For the Hopfield cell the on-sheet (controlled-trajectory) windows were generated as in the
...
cell, $30$ optimal-trajectory initial conditions (the benchmark on-sheet count),
...
Gram has rank equal to the effective rank $r^\star=\effrank C\approx 3.4$
(Finding~\ref{find:effrank}, the Veronese bound of Proposition~\ref{prop:veronese}), so its
co-rank is $\dim\phi-r^\star$. Off-sheet windows supply the missing rows, each raising
$\rank G_n$ by at most one; reaching full rank therefore needs
\[
  n_{\mathrm{off}}\ \gtrsim\ \dim\phi-r^\star\ =\ \dim\phi\,\bigl(1+o(1)\bigr)
...
Hopfield and the Mackey--Glass cells.
\end{observation}
...
Hopfield cell with on-sheet data only, over five seeds (Table~\ref{tab:onsheet}), extending the
canonical on-sheet-only comparison of Finding~\ref{find:onsheet} to the nonlinear-delayed primary
testbed and tying it to the rank. Only the dimension-$5$ markovian critic --- whose dimension is
...
\toprule
$\tau$ ($I^{0}\!\to\! I^{\star}$) & rep & $\dim\phi$ & $n/\dim$ & $I$ (mean $\pm$ ci)\\
\midrule
...
the signature on every cell (Findings~\ref{find:controls} and~\ref{find:mg}). The signature's
...
cells \emph{when data are scarce} (its sample-efficiency advantage), and its channel growth hurts
...
(off/dim${}\gtrsim2$), the degree-two history window matches or beats the signature on every cell
...
plants: $H_1$ holds, measured and seed-stable, on every cell where the delay couples the
...
benchmark reports $H_1$ failing on an ill-conditioned cell, the controlled plain-least-squares
...
polynomial of the history window: every apparent win is an artefact of the history map being
starved of data at long delay, and is removed by more data, by conditioning, or by off-sheet
...
\emph{Each figure overlays, from the cell's deployment initial condition, the closed-loop state
...
starved fit ($n/\dim\phi_H=4.4$): the history window (orange, dimension $945$) diverges into a
saturating bang-bang control ($I=545$) while the signature (purple) and oracle (dashed) track,
so the signature appears to win. Right, the same data with Tikhonov conditioning
...
$x^{\star}=1$. At this off-sheet budget the signature leads ($I=0.013$ versus the history
...
(\texttt{h1h2\_rl\_lspi}). $H_1$ holds on the delay cells and fails on the Markovian negative
control, mirroring Table~\ref{tab:h1}.}\label{tab:rl}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lccc c}
\toprule
cell & $I_M$ & $I_H$ & $I_S$ & $H_1$\\
\midrule
...
Table~\ref{tab:rl} reproduces the $H_1$ verdict: history lowers $I$ on every delay cell and not
...
(representation-limited). Evidence that the learned $H_1$ failure on this cell is
...
\item \texttt{h1h2\_evaluation.py} --- oracle-half harness (cells, oracle, data protocol, plain
...
Table~\ref{tab:mainunified}): \texttt{main\_unified/} --- per-cell sub-studies
...
\item Canonical $H_1/H_2$ (5 cells $\times$ 5 seeds): \texttt{20260616\_161650\_on\_off\_sheet\_array}
...
complete for both cells (the last mg\_chaotic point, off/dim${}=4$, at $\sim10^{4}$ Pontryagin
