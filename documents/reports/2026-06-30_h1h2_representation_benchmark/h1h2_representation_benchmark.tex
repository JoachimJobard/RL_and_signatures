...
the closed-loop cost $J$: $H_1$, that a representation of the \emph{history} window improves
...
at $J=3.115$ (measured, five seeds). The second hypothesis does \emph{not} hold. Early
...
(Hopfield at $\tau=3$: $J=0.289$ versus $0.323$; Mackey--Glass limit cycle at off/dim${}=4$:
$J=0.0108$ versus $0.0126$). The mechanism is the effective-rank phenomenon of the companion
...
growth makes it the ill-conditioned representation (platoon: $J=0.227$ versus the history
...
A plant is a controlled delay differential equation on $\R^{d_x}$,
\begin{equation}\label{eq:plant}
\dot x(t)=f\big(x(t),\,x(t-\tau),\,u(t)\big),\qquad x(s)=x_{0}(s)\ \ (s\in[-\tau,0]),
\end{equation}
with control $u(t)\in\R^{d_u}$ and a quadratic running cost
$c(x,u)=x^{\top}Qx+u^{\top}Ru$, $Q\succeq0$, $R\succ0$, regulating to the origin (a shifted
equilibrium). The closed-loop performance of a feedback law is the accumulated cost
\begin{equation}\label{eq:cost}
J=\int_{0}^{T}c\big(x(t),u(t)\big)\,\mathrm dt
\end{equation}
from a fixed deployment initial condition, integrated by the environment's own Runge--Kutta
solver. $J$ is the operative quantity: every verdict in this report is a comparison of $J$.
\end{definition}
...
the augmented state $z=\big(x(t+s_{0}),\dots,x(t+s_{\nu})\big)\in\R^{d_x(\nu+1)}$ stacks the
state at those nodes. The delay equation \eqref{eq:plant} becomes the ordinary differential equation
$\dot z=Mz+\mathcal N u$, in which $M\in\R^{d_x(\nu+1)\times d_x(\nu+1)}$ collocates the transport
identity $\partial_{t}x=\partial_{s}x$ at the interior nodes and the linearised dynamics
$\dot x=A_{0}\,x(t)+A_{1}\,x(t-\tau)$ at the boundary node $s_{0}=0$, and
$\mathcal N=(B^{\top},0,\dots,0)^{\top}\in\R^{d_x(\nu+1)\times d_u}$ injects the control at that node. The
...
reference cost $J^{\ast}$ against which the learned $J$ is read.
...
closed-loop cost:
\begin{itemize}
\item $H_1$ (\emph{does history help?}): $J[\text{raw-history}]<J[\text{markovian}]$;
\item $H_2$ (\emph{does the signature structure help?}): $J[\text{signature}]<J[\text{raw-history}]$.
\end{itemize}
Each is recorded as holding on an example only if the contender lowers $J$ by more than a relative
...
$\underline x=(x_{t-i\Delta t})_{0\le i\le L-1}\in(\R^{d_x})^{L}$ of the last $L$ state samples at the
...
feature map $\varphi$, $V_{\theta}(\underline x)=\inner{\theta}{\varphi(\underline x)}$, with the three maps
\begin{enumerate}
\item \emph{markovian} $\varphi_{M}$: the degree-$2$ monomials of the \emph{current} state $x_{t}$
alone (no history);
\item \emph{raw-history} $\varphi_{H}$: the degree-$2$ monomials of the \emph{whole} sequence
$\underline x$ (its $Ln$ scalar entries);
\item \emph{signature} $\varphi_{S}$: the depth-$2$ path signature of the time-augmented sequence
$\big(i\Delta t,\,x_{t-i\Delta t}\big)_{0\le i\le L-1}$.
\end{enumerate}
$\varphi_{M}$ is the $H_1$ null (a state-only critic); $\varphi_{H}$ adds the history linearly in the
sequence; $\varphi_{S}$ adds the nonlinear path-functional structure. Their feature dimensions are
...
\Xi_{i}\sim\mathcal N(0,\mathrm{Id}),\quad \varepsilon=0.25
\]
...
$n_{\text{train}}=n_{\mathrm{on}}+n_{\mathrm{off}}>\dim\varphi$ in every reported fit; the ratios
$n_{\text{train}}/\dim\varphi$ and $n_{\mathrm{off}}/\dim\varphi$ are reported because they are the
binding quantities for $H_2$.
\end{definition}

\begin{definition}[The two estimators]\label{def:estimator}
Let $\Phi\in\R^{n_{\text{train}}\times\dim\varphi}$ be the design matrix with rows
$\varphi(\underline x_{k})^{\top}$, let $y\in\R^{n_{\text{train}}}$ collect the oracle labels
...
\[
\hat\theta=\Big(G+\lambda\,\tfrac{\tr G}{\dim\varphi}\,\mathrm{Id}\Big)^{-1}\Phi^{\top}y,
\]
...
$\eta:=\dfrac{\inner{u_{\theta}}{u^{\ast}}}{\norm{u_{\theta}}\,\norm{u^{\ast}}}\in[-1,1]$, the
cosine of the angle between the deployed control $u_{\theta}$ and the optimal control $u^{\ast}$
stacked along the deployment trajectory ---
...
\item closed-loop cost $J$ (Equation~\eqref{eq:cost}) --- \emph{control}: the operative
quantity of comparison, read against the no-control cost $J^{0}$ and the oracle-feedback cost
$J^{\ast}$.
\end{itemize}
The three are ordered by stringency: $R^{2}\to1$ is necessary but far from sufficient for
$\eta\to1$, which is in turn necessary but not sufficient for $J\to J^{\ast}$. The gap between
$R^{2}=1$ and a large $J$ is the value/gradient separation of Section~\ref{sec:mechanism}.
...
$\psi(\xi)=\tanh(\kappa\xi)/\kappa$ on the delayed coupling, $\psi'(0)=1$ so the
linearisation $A=-\mathrm{Id},\,A_1=W$ is unchanged: $H_1$ is held fixed while the value becomes a
...
$\psi(\xi)=\xi+\kappa\xi^{3}$ on the coupling, with an instantaneous cubic confinement
$-d\,x^{3}$ to bound the plant; $\psi'$ \emph{grows} with amplitude, so history sensitivity
...
\caption{Example parameters and feature dimensions. $d_x$ is the state dimension, $\tau$ the
delay, $\Delta t$ the control step, $L$ the window length; $\dim\varphi$ from the measured
...
\toprule
example & $d_x$ & $\tau$ & $\Delta t$ & $L$ & $\dim\varphi_M$ & $\dim\varphi_H$ & $\dim\varphi_S$ & oracle\\
\midrule
...
raw-history dimension $\dim\varphi_H$ explodes with the \emph{delay} (Hopfield $90\to7625$ as
$L:6\to61$), the engine of the $H_2$ illusion of Section~\ref{sec:h2}. The signature dimension
$\dim\varphi_S$ is fixed in the delay but explodes with the \emph{state dimension}
...
$J=3.1151\pm0.0000$ for markovian, raw-history and signature alike (five seeds; the off-sheet
labels pin the history coefficients of $\varphi_H$ to zero because $V^{\ast}$ carries no history
...
On on-sheet-only data the same example gives raw-history $J=3.1939\pm0.0385$ (a $2.5\%$
...
\begin{table}[ht]\centering\small
\caption{Canonical $H_1/H_2$ table: closed-loop cost $J$, value-fit $R^{2}$ and gradient cosine
$\eta$, mean${}\pm{}$half-range over five seeds (oracle half, on${}+{}$off-sheet, plain least
squares). $J^{0}$ is the no-control cost, $J^{\ast}$ the oracle-feedback cost. Lower $J$ is
better. Bold marks the lower of raw-history vs markovian ($H_1$) and signature vs raw-history
($H_2$).}\label{tab:h1}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccc c}
\toprule
example ($J^{0}\!\to\! J^{\ast}$) & rep & $R^{2}$ & $\eta$ & $J$ & verdict\\
\midrule
...
raw-history \emph{diverges} ($J\sim10^{3}$, all five seeds) for the same reason --- its
...
markovian critic's deficit is visible already in the gradient cosine --- $\eta=0.05$ on
...
On mg\_chaotic the history critic \emph{diverges} ($J\sim10^{3}$), so the raw $H_1$ comparison
...
the high-dimensional fits diverge: raw-history rises to $J=117.6$ (mg\_limit\_cycle), $784.5$
(mg\_chaotic), and the markovian and signature critics to $J=235.9\pm90.1$ and $88.7\pm75.6$ on
...
The comparison $J[\text{signature}]<J[\text{raw-history}]$ is, as stated, ill-posed: it depends on
the data budget and the sampling law, which do not appear in it. Making them explicit separates two
distinct questions. Fix a feature map $\varphi:\mathcal X\to\R^{\dim\varphi}$ on the space of history
sequences $\mathcal X=(\R^{d_x})^{L}$, and let
\[
\mathcal H_\varphi=\big\{\,\underline x\mapsto\inner{\theta}{\varphi(\underline x)}:\theta\in\R^{\dim\varphi}\,\big\}
\]
be the induced linear value class. A critic $V$ is deployed through Doya's law \eqref{eq:doyalaw},
$u_{V}=\half R^{-1}B^{\top}\partial_{x}V$, and $J(V)$ denotes the resulting closed-loop cost. Because
the control is the \emph{gradient} of $V$, the operative metric is $J(V)$ --- not any
value-approximation error --- which is why a perfect value fit ($R^{2}=1$) is uninformative on its
own (Proposition~\ref{prop:kerG}). An estimator $\mathcal D\mapsto\hat\theta_\varphi(\mathcal D)$ on a
dataset of size $n$ (Definition~\ref{def:estimator}) gives the fitted critic $\hat V_\varphi^{\mathcal D}$.
The two representations are $\varphi_H$ (degree-two raw-history, $\dim\varphi_H=O(\tau^{2})$) and $\varphi_S$
(depth-two signature, $\dim\varphi_S$ fixed in $\tau$). The hypothesis then admits two well-posed forms.
...
H_2^{\mathrm{pop}}:\qquad \inf_{V\in\mathcal H_{\varphi_S}}J(V)\;<\;\inf_{V\in\mathcal H_{\varphi_H}}J(V).
\end{equation}
...
H_2^{(n)}:\qquad
\E_{\mathcal D\sim\mathcal P^{\otimes n}}\!\big[J(\hat V_{\varphi_S}^{\mathcal D})\big]\;<\;
\E_{\mathcal D\sim\mathcal P^{\otimes n}}\!\big[J(\hat V_{\varphi_H}^{\mathcal D})\big],
\end{equation}
...
\E\big[J(\hat V_\varphi)\big]-J^{\ast}=
\underbrace{\Big(\inf_{V\in\mathcal H_\varphi}J(V)-J^{\ast}\Big)}_{\text{approximation}}
+\underbrace{\Big(\E\big[J(\hat V_\varphi)\big]-\inf_{V\in\mathcal H_\varphi}J(V)\Big)}_{\text{estimation (conditioning)}}.
\end{equation}

The remainder of this section establishes, by measurement, that the \emph{approximation} terms of
$\varphi_H$ and $\varphi_S$ are comparable (both classes contain a near-optimal controller; degree-two
raw-history spans $V^{\ast}$ on the linear examples, $R^{2}\to1$), so $H_2^{\mathrm{pop}}$ does
\emph{not} hold; whereas the \emph{estimation} terms differ and scale as $\dim\varphi/n$, so
$H_2^{(n)}$ holds only in the under-sampled regime $n\lesssim \beta\,\dim\varphi_H$ and reverses for
$n\gtrsim2\,\dim\varphi_H$ (the off/dim${}\gtrsim2$ transition, Table~\ref{tab:controls},
Observation~\ref{obs:budget}). The naive inequality, when it holds, therefore measures the
estimation term at fixed budget --- sample efficiency ($\dim\varphi_S$ fixed versus
$\dim\varphi_H=O(\tau^{2})$) --- not expressivity; to ``collapse'' it under the controls is precisely
...
\texttt{h1h2\_tau\_sweep}). As $\tau$ grows the window $L$ and hence $\dim\varphi_H$ grow as
$O(\tau^{2})$, so $n_{\text{train}}/\dim\varphi_H$ falls; below ${\sim}5$ the history fit loses the
gradient ($\eta$ collapses) and $J$ \emph{diverges}, leaving the fixed-dimension signature as
the apparent winner.}\label{tab:tausweep}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{cccc cc cc}
\toprule
$\tau$ & $L$ & $\dim\varphi_H$ & $\tfrac{n}{\dim\varphi_H}$
 & $\eta_{H}$ & $J_{H}$ (raw-hist) & $\eta_{S}$ & $J_{S}$ (signature)\\
\midrule
...
\emph{lower} cost at short delay ($\tau=0.5,1$: $J_H=0.090,0.130$ versus $J_S=0.098,0.152$), and
the signature only overtakes it at $\tau=2,3$. But the overtaking coincides exactly with
$n_{\text{train}}/\dim\varphi_H$ falling through ${\sim}5$ to ${\sim}2$: at $\tau=2,3$ the history
\emph{diverges} ($J_H=544,383$; $\eta_H=0.17,0.12$), it is not out-performed. The apparent
...
\toprule
control & setting & $n/\dim\varphi_H$ & $\eta_H$ & $J_H$ & $J_S$\\
\midrule
\multicolumn{6}{l}{\emph{(a) more data --- Hopfield $\tau=2$, $\dim\varphi_H=945$ (\texttt{h1h2\_tau2\_control})}}\\
...
\midrule
\multicolumn{6}{l}{\emph{(c) off-sheet rebalancing --- Hopfield $\tau=3$, $\dim\varphi_H=2015$ (\texttt{h1h2\_tau3\_offsheet})}}\\
...
(a) Quadrupling the data at $\tau=2$ restores the history window from $J=544.6$ to $0.240$ and
its gradient cosine from $0.17$ to $0.98$, level with the signature's $0.229$.
(b) At fixed under-sampled data, Tikhonov conditioning alone restores it ($J:544.6\to0.238$ at
$\lambda=10^{-2}$) --- and \emph{degrades} the signature ($J:0.264\to6.887$), since the same
...
converge and \emph{beat} the signature: $J_H=0.289$ versus $J_S=0.323$, with $\eta_H=1.000$.
...
$n_{\mathrm{ic}}=30$ to $240$ moves the history window from $J=0.579$ (converging) to $J=1495$
(diverged), at $n/\dim\varphi_H=15.4$ --- a \emph{larger} sample
...
hardening nonlinearity at short delay (window $6$, $\dim\varphi_H=90$, well sampled at
$n/\dim\varphi_H=45.9$). Right: Mackey--Glass off-sheet scaling
(\texttt{h1h2\_mg\_offsheet}, single seed), off/dim${}\in\{1,2,4\}$. Lower $J$ in bold.}
...
On the Duffing example, where the delay is short ($\dim\varphi_H=90$) so the history window is
\emph{well} sampled at every $\kappa$, the history window has the lower cost at every
nonlinearity strength (Table~\ref{tab:generality}, left: $J_H<J_S$ throughout, $\eta_H\ge0.97$
versus $\eta_S\le0.91$). A genuinely nonlinear value functional is therefore \emph{not}
...
$J=0.240$ (off/dim 1, $\eta_H=0.17$, under-conditioned) through $0.017$ (off/dim 2) to
...
artefact: at off/dim${}=4$ it reverses. The chaotic example (window $69$, $\dim\varphi_H=2484$, the
hardest plant) overturns \emph{sooner}: under-conditioned at off/dim 1 ($J_H=0.193$ versus the
signature's $0.0179$), the history window already \emph{beats} the signature at off/dim 2
($J_H=0.0143$ versus $0.0184$, $\eta_H=0.87$) and \emph{plateaus} there
($J_H=0.0140$, $\eta_H=0.90$ at off/dim${}=4$; each such point requires $\sim10^{4}$ Pontryagin
...
statements are reproduced and cited here, not re-proved (the density theorem is a classical result,
attributed there to Arribas and to Hambly--Lyons), with the measured part kept in a separate
\textnormal{Empirical finding}.}

\subsection{The proven part}

\begin{theorem}[Density of linear functionals of the signature; Arribas 2018]\label{thm:density}
Let $\mathcal X$ be a set of continuous bounded-variation paths $[0,T]\to\R^{1+d}$, time-augmented
(each carrying a monotone time coordinate), compact for the $1$-variation topology, and containing
no two distinct tree-like-equivalent paths. Then for every $F\in C(\mathcal X;\R)$ and every
$\varepsilon>0$ there exist a truncation level $L\in\N$ and a linear functional $\ell$ of the
depth-$L$ signature such that
\[
\sup_{X\in\mathcal X}\big|F(X)-\inner{\ell}{\sig_{\le L}(X)}\big|<\varepsilon .
\]
\end{theorem}

\begin{remark}[Existence, not recovery]\label{rem:density}
Theorem~\ref{thm:density} is an existence statement in
$\big(C(\mathcal X),\norm{\cdot}_{\infty}\big)$: it asserts that an $\varepsilon$-approximant $\ell$
\emph{exists}, but bounds neither its norm, nor the stability of recovering it from a finite sample,
nor any derivative of $F$. The obstructions of this section --- the effective-rank degeneracy
(Proposition~\ref{prop:veronese}) and the value/gradient gap (Proposition~\ref{prop:kerG}) --- all
live in that gap.
\end{remark}
...
bound, with $L=2$, governs the degree-two raw-history covariance. (Companion note,
...
($R^{2}=1$) is compatible with an arbitrary control. (Companion note, Proposition~6.2.)
\end{proposition}
...
control --- is free, so $J$ diverges. Off-sheet relabelling (Definition~\ref{def:datamode})
shrinks $\ker G_{n}$ and restores identifiability. The natural objection is that exploration should
do the same; the next two statements show why, on a regulated plant, it does not --- and isolate the
one thing off-sheet design does that no exploration can.

\begin{proposition}[Exploration amplitude-invariance; companion note]\label{prop:lyapunov}
For a Hurwitz closed loop with generator $A$, the stationary state covariance under exploration of
noise covariance $\Sigma\succeq0$ is the solution $P(\Sigma)$ of $AP+PA^{\top}+\Sigma=0$. For a fixed
noise shape $\Sigma_{0}$, the effective rank $\effrank P(\sigma^{2}\Sigma_{0})$ and the condition
number $\cond P(\sigma^{2}\Sigma_{0})$ are independent of the exploration amplitude $\sigma$.
(Companion note, Proposition~4.1; the map $\Sigma\mapsto P(\Sigma)$ is linear, so
$P(\sigma^{2}\Sigma_{0})=\sigma^{2}P(\Sigma_{0})$, and $\effrank,\cond$ are degree-zero homogeneous.)
\end{proposition}

\begin{observation}[Off-sheet design versus exploration]\label{obs:explore}
Off-sheet perturbation and exploration pursue the same end --- populating the directions transverse
to the optimal trajectory, on which the value gradient is otherwise unconstrained
(Proposition~\ref{prop:kerG}) --- yet they are not interchangeable, and the distinction is
structural.
\begin{enumerate}
\item \emph{Amplitude does not condition.} By Proposition~\ref{prop:lyapunov} the stationary
distribution reached by isotropic exploration ($\Sigma_{0}=\mathrm{Id}$) has an effective rank set by
the closed-loop spectrum, not by the amplitude $\sigma$; with the Veronese bound
(Proposition~\ref{prop:veronese}) the induced feature Gram inherits the same bounded effective rank
as the on-sheet attractor. Increasing isotropic exploration therefore cannot lift $\ker G_{n}$ ---
the refuted remedy ``add more noise''.
\item \emph{Shape can; exploration is not futile.} The invariance is in the \emph{amplitude} at a
fixed noise shape; an anisotropic exploration $\Sigma_{0}\neq\mathrm{Id}$, chosen to excite the weak
directions, yields a different $P(\Sigma_{0})$ and \emph{can} change the effective rank. What fails is
the naive isotropic-amplitude form, not directed exploration.
\item \emph{Off-sheet decouples the query law from the flow.} The support of any exploration-driven
occupancy measure lies in the reachable set $\mathcal R$ of the closed loop: every state it labels is
one the dynamics can produce. Off-sheet relabelling instead evaluates $V^{\ast}$ at query points
$\underline x'=\underline x+\varepsilon\,\Xi$ (Definition~\ref{def:datamode}) that need not lie in
$\mathcal R$ --- the oracle solves the control problem from $\underline x'$ as an initial condition,
whatever its reachability. Off-sheet is thus state-space sampling that the stabilising loop does not
contract, and it can populate directions transverse to $\mathcal R$ that no exploration supported on
$\mathcal R$ can reach.
\end{enumerate}
The operative lever is off-manifold \emph{design}; its one component that no exploration can
replicate --- querying the value \emph{off} the reachable set --- is structural and requires an
oracle or a model, not a larger exploration amplitude.
\end{observation}
...
\toprule
$\tau$ & $L$ & $\dim\varphi_H$ & $\effrank\,C_H$ & $\dim/\effrank$ & $\effrank\,C_S$\\
\midrule
...
\begin{axis}[
  ...
  axis y line*=left, ymode=log, ylabel={nominal dimension $\dim\varphi_H$ (log)},
  ymin=150, ymax=12000,
]
...
\addlegendimage{solid, color=black, mark=*, line width=1.0pt}\addlegendentry{$\dim\varphi_H$ (raw-history)}
\addlegendimage{dotted, color=black!55, mark=square, line width=1.0pt}\addlegendentry{$\dim\varphi_S$ (signature, fixed)}
\end{axis}
\end{tikzpicture}
...
co-rank is $\dim\varphi-r^\ast$. Off-sheet windows supply the missing rows, each raising
$\rank G_n$ by at most one; reaching full rank therefore needs
\[
  n_{\mathrm{off}}\ \gtrsim\ \dim\varphi-r^\ast\ =\ \dim\varphi\,\bigl(1+o(1)\bigr)
  \qquad (\dim\varphi\to\infty).
\]
The off-sheet perturbations are of small amplitude, hence non-generic, so each contributes less
than a full unit of rank and the measured threshold is $n_{\mathrm{off}}\gtrsim \beta\,\dim\varphi$ with
$\beta\approx 2$ (the off/dim${}\approx 2$ transition of Table~\ref{tab:controls}), not $c=1$.
Since the signature dimension is fixed while the degree-two history dimension grows as
$\dim\varphi_H=O(\tau^{2})$ (window length $L=O(\tau)$, $\dim\varphi_H=O((Ln)^{2})$),
\[
...
close to the attractor rank --- is identifiable and stabilises the plant ($J=0.13$ at $\tau=0.5$
and $0.64$ at $\tau=2$, against the oracle $0.09$ and $0.21$); both high-dimensional
representations diverge ($J\sim3$--$4\times10^{2}$, all five seeds), exceeding even the no-control
cost ($J^{0}=9.85$ and $33.5$). The decisive case is $\tau=0.5$, where raw-history is
...
\begin{table}[ht]\centering\small
\caption{Hopfield on-sheet-only (no off-sheet data): closed-loop $J$, mean${}\pm{}$half the $95\%$
...
raw-history --- above even the no-control cost $J^{0}$. At $\tau=0.5$ raw-history has $n/\dim=40$,
so the divergence is the rank degeneracy, not a sample shortage.}\label{tab:onsheet}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{cl ccc}
\toprule
$\tau$ ($J^{0}\!\to\! J^{\ast}$) & rep & $\dim\varphi$ & $n/\dim$ & $J$ (mean $\pm$ ci)\\
\midrule
...
\begin{finding}[The signature is sample-efficient but not exempt from the off-sheet precondition]\label{find:sig-nooff}
Quantitatively the signature's frugality is its over-parameterisation ratio
$\dim\varphi_S/\effrank C_S\approx12$, \emph{fixed} in the delay (Table~\ref{tab:effrank}: dimension
$12$--$30$ over an on-manifold rank $\approx2.6$), against raw-history's $83$--$2180$; by the co-rank
bound (Observation~\ref{obs:budget}) its off-sheet budget for identifiability is $O(1)$, not
$O(\tau^{2})$. On the low-state-dimension examples this frugality is extreme: \emph{with no off-sheet
data at all} the signature already attains its full on${}+{}$off-sheet cost on both Mackey--Glass
examples ($J=0.015$--$0.019$) and on linear\_dde --- exactly where raw-history \emph{diverges}
($J=118$--$785$), Table~\ref{tab:sig-nooff}. But the signature is not exempt from the identifiability
obstruction: its dimension ($12$--$30$) still exceeds the on-manifold rank ($\approx3.4$), so
$\ker G_{n}\ne\{0\}$ on-sheet-only, and on the high-state-dimension platoon it too \emph{diverges}
without off-sheet data ($J=88.7$ versus $0.23$ with it; the Hopfield examples behave likewise,
Table~\ref{tab:onsheet}). Whether the signature's undetermined off-manifold gradient happens to
stabilise is the example-dependent accident of Proposition~\ref{prop:kerG}: the signature is more
sample-efficient, not immune.
\end{finding}

\begin{table}[ht]\centering\small
\caption{On-sheet-only (no off-sheet data) versus on${}+{}$off-sheet closed-loop cost $J$, per
example (canonical five-seed oracle half). Without off-sheet data the signature attains its
on${}+{}$off cost on the low-state-dimension examples (linear\_dde, both Mackey--Glass) --- where
raw-history \emph{diverges} --- but diverges itself on the platoon; the Hopfield examples behave like
the platoon (Table~\ref{tab:onsheet}).}\label{tab:sig-nooff}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lcc cc}
\toprule
example & signature on-sheet & signature on${+}$off & raw-history on-sheet & raw-history on${+}$off\\
\midrule
linear\_dde       & $0.166$           & $0.184$  & $0.152$ & $0.152$\\
markovian         & $3.116$           & $3.115$  & $3.194$ & $3.115$\\
mg\_limit\_cycle  & $\mathbf{0.0154}$ & $0.0128$ & $117.6$ & $0.036$\\
mg\_chaotic       & $\mathbf{0.0190}$ & $0.0180$ & $784.5$ & $1541$\\
...
on the high-state-dimension platoon (where $\eta_S=0.32$ and $J_S=0.227$ exceed even the
...
ill-conditioned gradient ($\eta_S=0.32$, condition number ${\sim}10^{14}$) producing a wrong
...
under-sampled fit ($n/\dim\varphi_H=4.4$): the history window (orange, dimension $945$) diverges into a
saturating bang-bang control ($J=545$) while the signature (purple) and oracle (dashed) track,
so the signature appears preferable. Right, the same data with Tikhonov conditioning
($\lambda=10^{-2}$): the history window is restored to $J=0.238$ and tracks the oracle, while the
identical ridge over-damps the signature ($J=6.89$). The cure is conditioning, not capacity
...
$J=5.23$), while the history window (orange) and signature (purple) both regulate to
$x^{\ast}=1$. At this off-sheet budget the signature leads ($J=0.013$ versus the history
...
\begin{table}[ht]\centering\small
\caption{Learning half (damped LSPI), closed-loop $J$, single seed
...
Table~\ref{tab:rl} reproduces the $H_1$ verdict: history lowers $J$ on every delay example and not
