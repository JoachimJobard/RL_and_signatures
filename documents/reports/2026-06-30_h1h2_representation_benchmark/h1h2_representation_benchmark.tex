...
\[
\hat\theta=\Big(G+\lambda\,\tfrac{\tr G}{\dim\phi}\,\mathrm J\Big)^{-1}\Phi^{\top}y,
\]
...
\item gradient cosine $\eta:=\cos\!\big(u_{\theta},u^{\ast}\big)\in[-1,1]$ ---
...
\begin{proof}[Proof (the note)]
The map $\Sigma\mapsto P(\Sigma)=\int_{0}^{\infty}e^{tA}\,\Sigma\,e^{tA^{\top}}\,\mathrm dt$ is linear,
so $P(\sigma^{2}\Sigma_{0})=\sigma^{2}P(\Sigma_{0})$; $\effrank$ and $\cond$ are homogeneous of degree
zero in the eigenvalues and are therefore invariant under the positive scaling $P\mapsto\sigma^{2}P$.
\end{proof}
